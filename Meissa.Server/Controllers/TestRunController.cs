...
public class TestRunController : Controller
{
    ...
    public async Task<IActionResult> GetTestRunAsync([FromBody] Guid id)
    {
        try
        {
            var testRun = (await _meissaRepository.GetAllQueryWithRefreshAsync<TestRun>().ConfigureAwait(false)).FirstOrDefault(x => x.TestRunId.Equals(id));
            if (testRun == null)
            {
                return NotFound();
            }
    ...
    public async Task<IActionResult> GetTestRunsAsync()
    {
        try
        {
            var testRuns = await _meissaRepository.GetAllQueryWithRefreshAsync<TestRun>().ConfigureAwait(false);
    ...
    public async Task<IActionResult> CreateTestRunAsync([FromBody] TestRunDto testRunDto)
    {
        ...
        var result = await _meissaRepository.InsertWithSaveAsync(testRun).ConfigureAwait(false);
    ...
    public async Task<IActionResult> UpdateTestRunAsync([FromBody] KeyValuePair<Guid, TestRunDto> updateObject)
    {
        ...
        var entityToBeUpdated = (await _meissaRepository.GetAllQueryWithRefreshAsync<TestRun>().ConfigureAwait(false)).FirstOrDefault(x => x.TestRunId.Equals(updateObject.Key));
