...
public class TestRunsCleanerController : Controller
{
    ...
    public TestRunsCleanerController(MeissaRepository repository) => _meissaRepository = repository;
    ...
    public async Task<IActionResult> DeleteOldTestRunDataByTestRunIdAsync([FromBody] Guid id)
    {
        try
        {
            ...
            if (testRun != null)
            {
                ...
                _meissaRepository.DeleteRange(testRunLogs);
                await _meissaRepository.DeleteWithSaveAsync(testRun).ConfigureAwait(false);
    ...
    public async Task<IActionResult> DeleteOldTestRunsDataAsync()
    {
        try
        {
            var testRuns = (await _meissaRepository.GetAllQueryWithRefreshAsync<TestRun>().ConfigureAwait(false)).Where(r => r.Status != TestRunStatus.InProgress || (r.Status == TestRunStatus.InProgress && r.DateStarted < DateTime.Now.AddDays(1)));
