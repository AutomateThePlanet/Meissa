...
public class TestRunAvailabilityController : Controller
{
    ...
    public async Task<IActionResult> GetLastTestRunAvailabilityForTestRun([FromBody] Guid id)
    {
        try
        {
            var testRunAvailability = (await _meissaRepository.GetAllQueryWithRefreshAsync<TestRunAvailability>().ConfigureAwait(false)).LastOrDefault(x => x.TestRunId.Equals(id));
    ...
    public async Task<IActionResult> GetTestRunAvailability([FromBody] int id)
    {
        try
        {
            var testRunAvailability = await _meissaRepository.GetByIdAsync<TestRunAvailability>(id).ConfigureAwait(false);
            if (testRunAvailability == null)
            {
    ...
    public async Task<IActionResult> GetTestRunAvailabilities()
    {
        try
        {
            var testAvailabilities = await _meissaRepository.GetAllQueryWithRefreshAsync<TestRunAvailability>().ConfigureAwait(false);
    ...
    public async Task<IActionResult> UpdateTestRunAvailabilityAsync([FromBody] KeyValuePair<int, TestRunAvailabilityDto> updateObject)
    {
        ...
        var entityToBeUpdated = await _meissaRepository.GetByIdAsync<TestRunAvailability>(updateObject.Key).ConfigureAwait(false);
    ...
    {
        var entityToBeRemoved = await _meissaRepository.GetByIdAsync<TestRunAvailability>(id).ConfigureAwait(false);
